...
É recomendado rodar o comando  `rails db:seed`  no seu terminal para popular o banco de dados com alguns models pré-cadastrados. Dessa forma, você terá acesso a dois logins de usuários, um comum e um administrador, que possuiem atribuições diferentes. Ao clicar no botão 'Fazer Login' localizado na barra de navegação da página inicial, você poderá utilizar as seguintes credenciais para se autenticar como usuário comum:
...
Promoção vencida ou cupom com uso máximo atingido:
...
```json
   {
     "status": "Cupom válido.",
     "discount": "valor do desconto - decimal"
   }
```
...
```
  ...
  Caso a requisição envie parâmetros inválidos, retorna status `412` e será exibida uma mensagem de erro em formato JSON:
